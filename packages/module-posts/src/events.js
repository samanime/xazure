...
import { findModule } from '@xazure/utils';
...
export default ({ config: { modules }, module: { publicPath, postTypes, routes: postRoutes }, a, mongoose }) => ({
  init: async previous => {
    await previous;
  ...
  getRoutes: async previous => {
    ...
    const { routes = {} } = args;
    const types = Object.keys((await a('getTypes')).types);

    return Object.assign(args, { routes: Object.assign(routes, types
      .reduce((r, type) => Object.assign(r, { [postRoutes[type] || `/${type}/:slug`]: [`post-${type}`, 'post'] }),
    {})) });
  },
