...
export default async () => {
  ...
  return {
    ...
    methods: {
      ...
      async getUser() {
        this.user = await (await fetch(`${authApiPath}/user`, { method: 'GET', credentials: 'same-origin' })).json();
      ...
      async confirmDelete() {
        this.showConfirmDelete = false;
        document.body.classList.remove('modal-open');
        const status = (await fetch(`${apiPath}/${this.id}`, { method: 'DELETE', credentials: 'same-origin' })).status;
      ...
      async onSubmit() {
        const response = await fetch(`${apiPath}${this.id ? `/${this.id}` : ''}`,
          { method: 'POST', body: JSON.stringify(this.post), headers, credentials: 'same-origin' });
